...
**The first six chapters in Day 1 of the training event. From the seventh chapter (session) onwards we are focusing on Day 2 of the training.**
...
This is also a good point to take a short break before we continue with the exrcise.
...
1. Create a **group** variable by recoding the **participant** variable. Participants with value below 2000 should be in the **control** group. Participants with value above 2000 should be in the **gambling** group.  
2. Create a variable that will denote the type of stimulus (call it **salience**). This information is included in the **Image** variable. If an image starts with **G** then it is a **gambling** image, if it starts with **N** then it is a **neutral** image.  
3. Keep only the correct responses, this information is in the  **stroop_resp.corr** variable. Correct responses are recorded with **1**.  
4. Discard the first two trials per participant. This information is in the **stroop_trials.thisTrialN** variable.  
5. Keep only the responses with a reaction time between 300 and 3000ms. This information is in the **stroop_resp.rt** (which is in seconds though).  
6. A variable with response times in ms would make more sense. Call it **RT**.  
7. Aggregate your **RT** by **participant**, **group**, and **salience**.  


<!--chapter:end:06-Bringing-Everything-Together-Part-I.Rmd-->
...
<!--chapter:end:07-Two-Way-Anova.Rmd-->
...
That worked fine, even though we have some missing values

## Let's see now if we have any significant correlation
## between the stroop difference and the frequency of games

## But first lets do a simple scatterplot


<!--chapter:end:08-Restructuring-and-Merging.Rmd-->
